...
contract Shots is Ownable {
    ...
    function pendingMok(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 MOKReward = multiplier.mul(MOKPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accMokPerShare = accMokPerShare.add(MOKReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accMokPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 MOKReward = multiplier.mul(MOKPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        MOK.mint(devaddr, MOKReward.div(10));
        MOK.mint(address(drink), MOKReward);
        pool.accMokPerShare = pool.accMokPerShare.add(MOKReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    
    function getPendings(uint256 _pid) public onlyOwner returns(uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accMokPerShare).div(1e12).sub(user.rewardDebt);
        emit Display(pending);
        return pending;
    }

    // Deposit LP tokens to Shots for MOK allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accMokPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                safeMokTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
    ...
    function enterStaking(uint256 _amount) public {
        ...
        drink.mint(msg.sender, _amount);
        emit Deposit(msg.sender, 0, _amount);
    }

    // Withdraw MOK tokens from STAKING.
