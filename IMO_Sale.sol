...
contract IMOSale is Ownable {
    ...
    uint256 public maxSupply = 3000000000000000000000000;
    ...
    */
    ...
    function investBNB(address _investor) public payable returns(bool status){
        ...
        require(msg.value >= minInvestment && msg.value <= maxInvestment, 'MOK Not Sold Below 0.01 BNB And Above 100 BNB!');
        require(totalMOKDistributed <= maxSupply, 'Out of MOK in Pre-Sale!');

        uint256 res1;
        uint256 res2;
        (res1, res2) = getBNBinBUSD();
        bnbPrice = res1.div(res2); // Init from reserves pair
        uint256 busdPrice = res2.div(res1); // Get BNB per BUSD

        if(investors[_investor]._name != address(0)){ // Checking for validations
            uint256 _totalInvested = busdPrice.mul(investors[_investor]._busdInvested) + investors[_investor]._bnbInvested; // Not more than 50 BNB Investment!
            require(_totalInvested < 50000000000000000000, 'Whoa! Thats a lot of investment from you! :)');
        }
        ...
        if(isMaintained){
            factory.transfer(amountInvested.mul(10).div(100)); // SafeTransfer To Factory Address
            amountInvested -= amountInvested.mul(10).div(100); // Fee Setter
        }

        uint256 mokDistributed = amountInvested.mul(bnbPrice.mul(mokPerBNB).div(dividend)); // MOK Calculated
    ...
    */
    ...
    function getMinBUSD() public view returns(uint256){
        return bnbPrice.mul(minInvestment);
    }

    function getMaxBUSD() public view returns(uint256){
        return bnbPrice.mul(maxInvestment);
    }
    ...
    */
    ...
    function investBUSD(address _investor, uint256 _amount) public returns(bool status){
        ...
        */
        ...
        uint256 res2;
        (res1, res2) = getBNBinBUSD();
        bnbPrice = res1.div(res2);
        uint256 busdPrice = res2.div(res1);

        require(_amount > minInvestment.mul(bnbPrice) && _amount < maxInvestment.mul(bnbPrice), 'MOK Not Sold Below 0.01 BNB And Above 100 BNB!');

        if(investors[_investor]._name != address(0)){
            uint256 _totalInvested = busdPrice.mul(investors[_investor]._busdInvested) + investors[_investor]._bnbInvested;
            require(_totalInvested < 50000000000000000000, 'Whoa! Thats a lot of investment from you! :)');
        }
        ...
        */
        ...
        if(isMaintained){
            busd.safeTransfer(factory, _amount.mul(1).div(100)); // If is maintained, fee setter
            amountInvested -= _amount.mul(10).div(dividend)
        }
        bool _status = false;

        uint256 mokDistributed = amountInvested.mul(mokPerBNB).div(dividend); // Calculating `MOK` Tokens minted per investment
